...
const AdminOrganizersPage = () => {
  ...
  const fetchOrganizers = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get("/api/v1/user/admin/organizers");
      if (res.data?.success) {
        setOrganizers(res.data.organizers || []);
      }
    } catch (e) {
      toast.error(e.response?.data?.message || "Failed to fetch organizers");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-950/20 border-b dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Organizer Details</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">Manage all registered organizers on the platform.</p>
        </div>
      </div>
      <main className="max-w-7xl mx-auto p-8">
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6 mb-6 border dark:border-gray-800">
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              <span className="font-medium text-gray-700 dark:text-gray-200">{filtered.length}</span> organizers found
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-12 text-center border dark:border-gray-800">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">No organizers found</h3>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                <thead className="bg-gray-50 dark:bg-gray-800/50">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Organizer</th>
                    ...
                    <tr key={u._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center text-purple-600 dark:text-purple-400 font-bold overflow-hidden">
                            {u.profileImage ? (
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Link
                            to={`/profile/${u._id}`}
                            className="p-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 hover:text-purple-600 dark:hover:text-purple-400 transition-all"
                            title="View Profile"
                          >
                          ...
                          <button
                            ...
                            title={u.isBlocked ? "Unblock Organizer" : "Block Organizer"}
                          >
                            {u.isBlocked ? (
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
